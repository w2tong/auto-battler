...
export default function CharacterSelectCard({ name, charClass, level, exp, index, selected, setSelected }: CharacterSelectCardProps) {
    ...
    return (
        <button className={cn('hover:bg-card w-full', selected && 'bg-card')} onClick={() => setSelected(index)}>
            <div className='flex flex-row items-center p-2'>
                <ClassIcon className='w-[80px] h-[80px]' charClass={charClass} />
